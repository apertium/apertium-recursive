...
{
  wstring v = toWstring(requireAttr(node, (const xmlChar*) "pos"));
  if(v.size() == 0)
  {
    if(isBlank)
    {
          ...
          if(inOutput)
          {
            ...
            for(unsigned int i = 0; i < vec.size(); i++)
            {
              ...
              else
              {
                ...
                warn(rule, L"Rules " + to_wstring(other) + L" and " + to_wstring(curRule) + L" both match '" + vec[i] + L"', the earliest one will be used.");
              }
            }
          }
