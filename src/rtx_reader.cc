...
{
  // it seems like compiling this as a single rule should be possible,
  // but trying to build the relevant pattern kept giving a segfault
  // whenever anyone tried to access the symbol generated by
  // td.countToFinalSymbol() -D.S. 6/16/19
          ...
          it != attrDefaults.end(); ++it)
  {
    ...
    update->weight = -1.0;
    update->pattern.push_back(vector<wstring>(1, L""));
    update->pattern[0].push_back(L"*." + undef);
    ...
    cur += (wchar_t)1;
    cur += SETCLIP;
    cur += compileClip(L"whole", 1, L"tl");
    cur += OUTPUT;
    update->compiled = cur;
  }
}
